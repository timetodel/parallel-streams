...
# Папка реестра, из которой список ДОСТАНУТ, если он понадобится. Не путать с самим списком:
# здесь лежит только адрес, чтения ещё не было.
$script:WaveBoardKnownWavesSource = ''
# Заведён ли список. Отдельным признаком, а не проверкой «список пуст»: пустой список —
# законный ответ (реестра нет, заявок нет), и без признака его переспрашивали бы у диска
# на каждом разборе адреса — ровно та плата ни за что, от которой этот механизм и уходит.
$script:WaveBoardKnownWavesLoaded = $false

function Set-KnownWaves {
    param([string[]]$Keys)
    $script:WaveBoardKnownWaves = @($Keys | Where-Object { $_ } | Select-Object -Unique)
    $script:WaveBoardKnownWavesLoaded = $true
}

function Set-KnownWavesSource {
    param([string]$Dir)
    # Отложенная форма: запоминает, ГДЕ лежит реестр, но не читает его.
    #
    # ‼️ Зачем откладывать. Читать реестр наперёд стоило 630 мс при 79 заявках — и платилось это
    # на КАЖДОМ обращении к доске, включая те, где список волн не нужен вовсе (спросить путь
    # доски) и те, где режим тут же читает реестр сам и список переставляет.
    #
    # Замер 06.09.2026 на рабочем репозитории (медиана семи чередующихся заходов): спросить путь
    # доски 1510 мс → 513, показать доску 568 → 485. Плата растёт вместе с реестром, а он только
    # копится: 79 заявок здесь — это два месяца работы.
    #
    # Сторожа доставки это НЕ касается, хотя он и есть самый частый вызывающий (каждый ход каждой
    # вкладки): он читает заявки по делу и список ставит из них же — замер до и после дал 552 и
    # 580 мс, то есть неотличимо. Проверено намеренно: догадка «на каждом ходу платят полсекунды»
    # выглядела очевидной и оказалась неверной.
    $script:WaveBoardKnownWavesSource = $Dir
    $script:WaveBoardKnownWaves = @()
    $script:WaveBoardKnownWavesLoaded = $false
}

function Get-KnownWaves {
    # Первое обращение достаёт список из реестра, дальше отвечает уже прочитанным: разбор адреса
    # зовут по разу на КАЖДУЮ запись доски, и без памяти о прочитанном отсрочка обернулась бы
    # чтением папки на каждую запись — то есть хуже, чем было.
    if (-not $script:WaveBoardKnownWavesLoaded -and $script:WaveBoardKnownWavesSource) {
        Set-KnownWavesFromRegistry -Dir $script:WaveBoardKnownWavesSource
    }
